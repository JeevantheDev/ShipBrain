...
import { AlertTriangle, CheckCircle2, ExternalLink, GitPullRequest, Rocket, X } from "lucide-react";
...
type Trace = {
  ...
  production_deployment?: { url?: string; status?: string; tag?: string; releaseTag?: string; runId?: string | number; run_id?: string | number; workflowRunId?: string | number } | null;
...
const columns: BoardColumn[] = [
  {
    ...
    matches: (trace) => trace.status === "failed" || Boolean(trace.pending_action)
  },
  {
    id: "development",
    title: "Development",
    ...
    matches: (trace) => !trace.pending_action && ["draft", "ready_for_review", "approved"].includes(trace.status)
  },
  {
    id: "preview",
    title: "Preview",
    ...
    matches: (trace) => !trace.pending_action && ["merged_develop", "preview_live"].includes(trace.status)
  },
  {
    id: "production",
    title: "Production",
    ...
    matches: (trace) => !trace.pending_action && ["release_pending", "merged_main", "production_live"].includes(trace.status)
  },
  {
    id: "done",
    title: "Done",
...
function primaryActionLabel(trace: Trace) {
  if (trace.pending_action?.description) return trace.pending_action.description;
  if (trace.preview_deployment?.url && trace.status === "preview_live") return "Preview is live. Promote to production when approved.";
  if (trace.production_deployment?.url) return "Production deployment is available.";
...
function deploymentRunId(deployment?: Trace["preview_deployment"] | Trace["production_deployment"]) {
  const direct = deployment?.runId ?? deployment?.run_id ?? deployment?.workflowRunId;
  return direct ? String(direct) : workflowRunIdFromUrl(deployment?.url);
}

function ciMonitorHref(trace: Trace) {
  const runId = deploymentRunId(trace.production_deployment) ?? deploymentRunId(trace.preview_deployment);
  return runId ? `/ci?run=${encodeURIComponent(runId)}` : "/ci";
}

function deploymentLabel(url?: string) {
  if (!url) return "Not available yet";
  return url.includes("/actions/runs/") ? "GitHub Actions workflow" : "Deployment URL";
}

export function ReleaseTraceBoard({ traces, eventsByTrace }: { traces: Trace[]; eventsByTrace: Record<string, TraceEvent[]> }) {
    ...
    <>
      <section className="release-board-shell">
        <div className="release-board">
          {grouped.map((column) => (
            <div className="release-board-column" key={column.id}>
                  ...
                  column.traces.map((trace) => (
                    ...
                    >
                      <div className="release-card-top">
                        <span className={`trace-type-pill ${trace.type}`}>{trace.type}</span>
                      ...
                      <div className="release-card-footer">
                        {trace.pending_action ? <AlertTriangle size={13} /> : <CheckCircle2 size={13} />}
                        <span>{trace.pending_action?.type ? statusLabel(trace.pending_action.type) : trace.current_phase ?? "tracked"}</span>
                      </div>
                    </button>
                  ))
                ) : (
      ...
      {selected ? (
        <div className="release-drawer-backdrop" role="dialog" aria-modal="true" onClick={() => setSelectedId(null)}>
          <aside className="release-drawer" onClick={(event) => event.stopPropagation()}>
            <div className="release-drawer-head">
              <div>
                <span className="eyebrow mono">{selected.type} · {shortId(selected.id)}</span>
                <h2>{selected.title}</h2>
                <p>{selected.repo_full_name}</p>
              </div>
              <button className="icon-btn" type="button" aria-label="Close release details" onClick={() => setSelectedId(null)}>
                <X size={16} />
              </button>
            </div>

            <div className="release-drawer-scroll">
              <section className={`release-detail-callout ${selected.pending_action ? "attention" : ""}`}>
                <div>
                  <span className={`status-pill ${stageClass(selected)}`}>
                    <span className="dot"></span>
                    {statusLabel(selected.status)}
                  </span>
                  <h3>{selected.pending_action?.type ? statusLabel(selected.pending_action.type) : selected.current_phase ?? "Release trace"}</h3>
                  <p>{primaryActionLabel(selected)}</p>
                </div>
                <TraceActions traceId={selected.id} pendingType={selected.pending_action?.type ?? null} status={selected.status} />
              </section>
              ...
              <section className="release-detail-section">
                ...
                <div className="release-link-grid">
                  {selected.draft_pr_number ? (
                    <a className="btn subtle" href={traceHref(selected, "draft") ?? "#"} target="_blank" rel="noreferrer">
                  ...
                  {selected.release_pr_number ? (
                    <a className="btn subtle" href={traceHref(selected, "release") ?? "#"} target="_blank" rel="noreferrer">
                      <GitPullRequest size={13} />
                  ...
                  {selected.reverse_sync_pr_number ? (
                    <a className="btn subtle" href={traceHref(selected, "sync") ?? "#"} target="_blank" rel="noreferrer">
                      <GitPullRequest size={13} />
                  ...
                  {selected.preview_deployment?.url ? (
                    <a className="btn subtle" href={selected.preview_deployment.url} target="_blank" rel="noreferrer">
                      <Rocket size={13} />
                      Open preview URL
                      <ExternalLink size={12} />
                    </a>
                  ...
                  {selected.production_deployment?.url ? (
                    <a className="btn subtle" href={selected.production_deployment.url} target="_blank" rel="noreferrer">
                      <Rocket size={13} />
                      Open production URL
                      <ExternalLink size={12} />
                    </a>
              ...
              <section className="release-detail-section">
                ...
                <div className="trace-detail-list">
                  <span>Branches</span><strong>{selected.source_branch ?? "n/a"} → {selected.target_branch ?? "n/a"}</strong>
              ...
              <section className="release-detail-section">
                <div className="section-label mono">
                  <span>Timeline</span>
                  <span className="count">{eventsByTrace[selected.id]?.length ?? 0}</span>
                </div>
