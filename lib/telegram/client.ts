  ...
  const response = await fetch(`${telegramApiBase}/bot${token}/setMyCommands`, {
    ...
    body: JSON.stringify({
      commands: [
        ...
        { command: "rollback_releases", description: "List releases available for rollback" },
        { command: "rollback", description: "Rollback to a previous release by tag" },
        { command: "rollback_status", description: "Show active rollback status" },
